...
function FeaturedProjectCard({ project, index = 0 }: ProjectProps) {
  ...
  const cardHeight = isLarge ? "min-h-[500px] md:min-h-[600px]" : "min-h-[280px] md:min-h-[290px]"

  return (
    <div
      className={cn(gridClasses[index] || "col-span-12 md:col-span-4")}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div
        className={cn(
          "relative h-full rounded-xl overflow-hidden border border-border bg-card transition-all duration-300 hover:border-primary/50 hover:shadow-lg",
          cardHeight
        )}
      >
        {/* Animated border */}
        ...
        <div className="relative h-full flex flex-col justify-end p-6 md:p-8">
          ...
          <div className="space-y-4">
            <h3 className={cn("font-bold tracking-tight", isLarge ? "text-3xl md:text-4xl" : "text-xl md:text-2xl")}>
              {project.title}
            </h3>

            <p className={cn("text-muted-foreground leading-relaxed", isLarge ? "text-base md:text-lg line-clamp-3" : "text-sm line-clamp-2")}>
